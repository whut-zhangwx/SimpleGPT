...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not self.flash:
            ...
            self.register_buffer("bias", torch.tril(torch.ones(config.time_step, config.time_step))
                                        .view(1, 1, config.time_step, config.time_step))
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert config.time_step is not None
        self.config = config

        self.transformer = nn.ModuleDict(dict(
            wte = nn.Embedding(config.vocab_size, config.embed_dim), # token_embedding
            wpe = nn.Embedding(config.time_step, config.embed_dim),  # position_embedding
            drop = nn.Dropout(config.dropout),
            blocks = nn.ModuleList([Block(config) for _ in range(config.n_layer)]), # blocks
    ...
    def forward(self, idx, targets=None):
        device = idx.device
        b, t = idx.size()
        assert t <= self.config.time_step, f"Cannot forward sequence of length {t}, block size is only {self.config.time_step}"
    ...
    def crop_time_step(self, time_step):
        # model surgery to decrease the block size if necessary
        # e.g. we may load the GPT2 pretrained model checkpoint (block size 1024)
        # but want to use a smaller block size for some smaller, simpler model
        assert time_step <= self.config.time_step
        self.config.time_step = time_step
        self.transformer.wpe.weight = nn.Parameter(self.transformer.wpe.weight[:time_step])
        for block in self.transformer.blocks:
            if hasattr(block.attn, 'bias'):
                block.attn.bias = block.attn.bias[:,:,:time_step,:time_step]
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        print("forcing vocab_size=50257, time_step=1024, bias=True")
        config_args['vocab_size'] = 50257 # always 50257 for GPT model checkpoints
        config_args['time_step'] = 1024 # always 1024 for GPT model checkpoints
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        cfg = self.config
        L, H, Q, T = cfg.n_layer, cfg.n_head, cfg.embed_dim//cfg.n_head, cfg.time_step
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            # if the sequence context is growing too long we must crop it at time_step
            idx_cond = idx if idx.size(-1) <= self.config.time_step else idx[:, -self.config.time_step:]
            # forward the model to get the logits for the index in the sequence
            logits, _ = self(idx_cond) # self.__call__
